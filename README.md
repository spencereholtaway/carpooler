...
A small multi-user carpool organizer: sign in with Google, post a trip as a driver, join a trip as a rider.

Stack: React + Vite (frontend), Firebase Auth (Google sign-in) + Firestore (database), hosted free on Netlify.
...
Open the local URL it prints and sign in with Google.
...
6. Back in Firebase console > Authentication > Settings > **Authorized domains**, add your `*.netlify.app` domain (otherwise Google sign-in will be blocked on the live site).

## How it works

- `src/firebase.ts` — Firebase app/auth/Firestore setup, reading config from env vars.
- `src/AuthContext.tsx` — Google sign-in/sign-out, exposes the current user via `useAuth()`.
- `src/useTrips.ts` — Firestore reads/writes for trips (create, live list, join, leave, delete).
- `src/NewTripForm.tsx` / `src/TripCard.tsx` — the two pieces of UI.
